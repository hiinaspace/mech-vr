...
func _initialize() -> void:
	...
	check(result.distance_to(aim) < 0.001 and world.target_hits == 0, "Reticle agrees with trajectory but hit waits for pulse travel")
	world.tick(.3, Transform3D.IDENTITY, far_shield)
	check(world.target_hits == 1, "Traveling pulse reaches target")
